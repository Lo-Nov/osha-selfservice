...
class SBPController extends Controller
{
    ...
    public function printReceipt(Request $request)
    {
      ...
      return view('nakuru-sbp.receipt')->with($this->data);
      }
    }
    ...
    public function registerBusinessForm() {
    	$url = $this->url .'permit/api/sbp/postal';
    	$postal_info = json_decode($this->get_curl($url));
      if(is_null($postal_info))
      {
        return redirect()->back()->withErrors('Something went wrong. Please try again.');
      }

      if($postal_info->status_code != 200)
      {
...
//      if($sub_info->status_code != 200)
      ...
      if($act_info->success === false)
      {
    	...
    	$url = $this->url . 'permit/api/sbp/documents';
    	$doc_info = json_decode($this->get_curl($url));
      if(is_null($doc_info))
      {
        return redirect()->back()->withErrors('Something went wrong. Please try again.');
      }
      ...
      if($doc_info->status_code != 200)
      {
    	...
    	$postalcodes = $postal_info->response_data;
    	$subcounties = $sub_info->data;
    	$business_activities = $act_info->data;
    	$documents = $doc_info->response_data;
    ...
    {
    	$url = $this->url . 'permit/api/sbp/postal_town';
    	$data = [
    		'code' => $id
    	];

    	$postal_info = json_decode($this->postal_curl($url, $data));
      if(is_null($postal_info))
      {
        return redirect()->back()->withErrors('Something went wrong. Please try again.');
      }
      ...
      if($postal_info->status_code != 200)
      {
    	...
    	return response()->json($postal_info->response_data);
    ...
    public function getActivityDetail($id)
    {
     ...
     public function registerTradeLicense(Request $request)
     {
      ...
      }elseif ($this->data['TradeLicense']->success !=true) {
          return redirect()->back()->withErrors($this->data['TradeLicense']->message);
      } else {
    ...
    public function printTradeBill($id)
    {
        ...
        $pdf = PDF::loadView('bills.trade-bill', $data);
    ...
    {
      // dd($request->all());
      $url = $this->url. 'permit/api/sbp/searchBusiness';
      $data =
       [
        'BusinessID' => $request->business_id,
      ];

      $business_info = json_decode($this->business_curl($url, $data));
      ...
      {
          return redirect()->back()->withErrors($business_info->message);
      }
      ...
      return view('sbp.update', ['business' => $business_info->response_data, 'subcounties' => $sub_info->response_data]);
    }
    ...
    public function renew(Request $request)
    {
      ...
      $validatedData = $request->validate([
        ...
        'ContactPersonTelephone1' => 'required',
        'IDDocumentNumber' => 'required',
      ...
      $url = $this->url . 'permit/api/sbp/renewal';

      $data = $request->all();
      // dd($data);
      $renew_info = json_decode($this->renew_curl($url,$data));
      ...
      $data = [
        'BillNo' => $renew_info->response_data->bill_number];
      ...
      return view('sbp.confirm-renew', $data);
    }
