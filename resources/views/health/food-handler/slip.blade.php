                ...
                <div class="service-form-container  d-flex flex-column-md animated col-12 p-0">
                    ...
                    <div class="col-lg-5 p-5 position-relative transactions-form-container d-flex">
                        <div class="col-12 p-0 the-transaction-form animated">
                            ...
                            @endif
                            <form class="transaction-form" method="post" action="{{ route('print-foodhandler-cert') }}">
                                @csrf
                                <div id="errors" class="alert alert-danger d-none"></div>
                                <div class="form-group col-sm-12 col-md-12 p-0 mt-2 mb-5">
                                    <label for="mpesa-phone" class=" ">National Id</label>
                                    <input type="text" class="form-control" id="plot" placeholder="Enter your national ID number" name="idNo" value="{{old('idNo')}}">
                                </div>
                                ...
                                <div class="col-12 px-0 text-uppercase mt-5 pt-5">
                                    <button id="btn-otp" type="submit" class="btn btn-primary text-white font-14 w-100 p-4 center mx-0"><span class="btn-text text-uppercase font-12"><i class="fa fa-print"></i> Print result sip</span> <i data-feather="arrow-right" class="ml-2 float-right pull-right"></i></button>
                                </div>
                            </form>
                        </div>
                            ...
                            <div class="">
                                ...
                                <div class="transactions-details-container  ">
                                    ...
                                    <form>
                                        ...
                                        <div class="col-12 px-0 text-uppercase mt-5 pt-5">
                                            <button id="check-otp" class="btn btn-primary text-white font-14 w-100 p-4 center mx-0"><span class="btn-text text-uppercase font-12">Check for Results</span> <i data-feather="arrow-right" class="ml-2 float-right pull-right"></i></button>
                                        </div>
                                    </form>
                                    <!--  -->
...
@section('scripts')
    ...
    <script type="text/javascript">
        $('#check-otp').click(function(e){
            ...
            e.preventDefault();
            $('#check-otp').text('Checking details...');
            $('#loader3').removeClass('d-none');

            var otp = $('input[name=pin]').val();
            $.ajax({
                ...
                success:function(data){
                    ...
                    if(data == "success"){
                        $('#check-otp').text('CHECK FOR CERTIFICATES');
                        var id = $('input[name=idNo]').val();
                        //alert(id);
