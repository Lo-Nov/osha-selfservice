...
<!--	  the main section-->
	   <section class="container px-5  m-0  ">
		   ...
		   <h6 class=" text-capitalize serial-init font-22 mb-0">{{$bill->Customer}}</h6>
		   <!-- <p class="m-0 p-0"><strong class="weight-600">Identifier: {{$bill->BillDetailNo}}</strong></p> -->
		   </div>
		   ...
		   <div class="px-5 mx-5">
            ...
            <tbody>
              ...
              <tr class=" ">
                ...
                <td scope="row" class="pl-0">{{$bill->AccountNo}}</td>
                <td>{{$bill->FeeDescription}}</td>
                <td class="text-uppercase txt-primary pr-0 text-right amount">{{number_format($bill->BillTotal)}}</td>
              </tr>
            </tbody>
          </table>
		   </div>
            ...
            <tbody class="border-bottom">
              <tr class="  pl-0">
                <td class="py-2 pl-0">
					<table class=" ">
						<tbody><tr>
								...
								<td class="py-0"><strong>367776 <br> {{$bill->PaymentCode}}</strong></td>
							</tr>
                ...
                <td class="text-uppercase text-right py-2 pr-0 position-relative">
					<strong class="txt-primary font-21 ">KES {{number_format($bill->BillTotal)}}</strong>
					<!-- <p class="warning text-danger position-relative dropright fixed-bottom note text-nowrap py-2"><small class="note"><strong>pay before 6 months</strong></small></p> -->
...
<!--		   footer-->
		    <div class="row mt-2">
				<div class="col-12 ">
					<p class="">
						<span>Billed By <strong>{{Session::get('resource')['username']}}</strong>||<strong>{{$bill->PaymentCode}}</strong>||</span>
        ...
        <div class="row">
          <div class="col-12">
				<div class="row px-5 greybg">
			  		 <div class="col-6 pl-5">
					<div class="header-title text-uppercase p-3 pt-3 mx-5 col-8 bg-primary text-white">
                <h2 class=""><strong>Food handlers bill</strong></h2>
                <p>file copy</p>
              </div>
				</div>
              <center class="col-3 text-nowrap text-right   pt-3">
				  <small class=" "><strong>Billing Date</strong></small>
					<p class="  font-22">{{date('d-m-Y', strtotime($bill->DateCreated))}}</p>
              </center>
...
<!--	  the main section-->
	   <section class="container px-5  m-0  ">
		   ...
		   <h6 class="text-capitalize  serial-init font-22 mb-0">{{$bill->Customer}}</h6>
		   <!-- <p class="m-0 p-0"><strong class="weight-600">Bill identifier: {{$bill->BillDetailNo}}</strong></p> -->
		   </div>
