                ...
                <div class="col-12 p-lg-5 p-sm-3 ">
                    ...
                    <div class="col-12 p-0 pb-3 transactions-actions d-none animated mt-5">
                      ...
                      <li class="font-14 m-0">1.Go to the M-PESA menu on your SIM Toolkit</li>
                       <li class="font-14 m-0">2.Under the Lipa na M-PESA option, select Pay Bill and enter <strong>175555</strong>  as the business no.</li>
                       <li class="font-14 m-0">3.Enter <strong id="bill_pay">{{ $TradeLicense->data->header[0]->billNo  }}</strong>  as the account no.</li>
                       <li class="font-14 m-0">4.Enter KES <strong id="pay_amount">{{ $TradeLicense->data->header[0]->billAmount  }}</strong>  as the amount.</li>
...
{{--                <a href="{{  route('food-hygiene-business-details') }}">Back</a>--}}
              <h2 class="mb-4 text-white">Get your Trade License Bill Details </h2>
            ...
            <div class="row animated bill-actions-container">
                  <div class="col-sm-12 col-md-6 col-lg-6">
                     <p class="text-capitalize mb-1"><strong>Application has been made succesfully</strong></p>
                      ...
                      <p class="mb-0 p-0 mt-5">Trade License total application fee is:</p>
                      <h4 class="mt-0 pt-0">Kes <span id="bill_amount">{{  number_format($TradeLicense->data->header[0]->billAmount,2) }}</span></h4>
                    <!--pay via mpesa now-->
                    ...
                    <div class="payment-actions">
                      ...
                      <input type="hidden" class="form-control col flex-grow-1 border-0" id="codeP" value="{{$TradeLicense->data->header[0]->billNo }}" name="idNo">

                      <button formtarget="_blank" class="btn btn-white font-12 text-primary border-2 text-capitalize btn-Trade-bill" id="btn-print">
                      ...
                      <div class="row mx-0 mt-4 mb-2 gray-bg p-4 animated health-mpesa position-relative d-none" id="example">
                         ...
                         <span class="close"> <i data-feather="x"></i></span>
                          ...
                          <form class="form-inline mpesa-form flex-grow-1">
                                ...
                                <input type="text" class="form-control col flex-grow-1 border-0" id="phoneNumber" placeholder="07..." name="phoneNumber" value="{{old('phoneNumber')}}">
                                <input type="hidden" class="form-control col flex-grow-1 border-0" id="paymentCode" value="{{  $TradeLicense->data->header[0]->billNo }}" name="paymentCode">
                                <input type="hidden" class="form-control col flex-grow-1 border-0" id="Amount" value="{{ number_format($TradeLicense->data->header[0]->billAmount)  }}" name="Amount">
                                <input type="hidden" class="form-control col flex-grow-1 border-0" id="accNo" value="{{ $TradeLicense->data->header[0]->billNo }}" name="accNo">
                                </div>
                  ...
                  <div class="col-sm-12 col-md-6 col-lg-6 p-5 bg-light">
                     <p class="text-capitalize mb-1"><strong>This is your bill information</strong></p>
                      ...
                      <ul>
                          ...
                          <li>Client: <i>{{ $TradeLicense->data->header[0]->payerName }}</i>  </li>
                          <li>Client No:<i>{{ $TradeLicense->data->header[0]->clientNo }}</i> </li>
                          <li>Bill Number:<i>{{ $TradeLicense->data->header[0]->billNo }}</i> </li>
                          <li>Description:<i> {{ $TradeLicense->data->details[0]->feeAccountDesc }} </i></li>
                          <li>Fiscal Year:<i> {{ $TradeLicense->data->header[0]->financialYear }} </i></li>
                          <li>Created Date:<i> {{ $TradeLicense->data->details[0]->dateCreated }} </i></li>
...
<script type="text/javascript" src="{{asset('js/jquery.min.js')}}"></script>
    ...
    <script type="text/javascript">
        $(".btn-Trade-bill").click(function(e){

          e.preventDefault();

          var codeP = $('#codeP').val();

          window.location = '<?php echo url('/print-trade-bill') ?>/'+codeP;
        } );
    </script>
        ...
        function checkagain(pay_id,recheck_count)
        {
          ...
          if(recheck_count===20){
                      ...
                      $('.payment-status').text('Unable to validate payment!');                    ;
                        ...
                        url :"<?php echo url('/get-overall-receipt') ?>/" +pay_id,
                        type: "GET",
                        dataType: "JSON",
                        success: function(data)
                        {
                            ...
                            if(data.success.success === true  && data.success.data.callback_returned === "PAID")
                            {
                                ...
                                setTimeout(function(){
                                    window.location = '<?php echo url('all-printables') ?>/'+pay_id;
                                }, 5000);
