        ...
        <span class="back-toform" title="back to transactions form"><i data-feather="arrow-left"  class="mr-3 float-left"></i></span><strong>Confirm your Transaction details</strong>
           ...
           <div class="transactions-details-container  ">
            <p class="m-0"><span>tenant</span></p>
             <strong>{{$houseDetails->CustomerSupplierName}}</strong>
             <hr>
             <p class="m-0"><span>House</span></p>
             <strong>{{$houseDetails->HouseTypeDescription}} {{$houseDetails->HouseNumber}}</strong>
             <hr>
             
            <p class="m-0"> <span>monthly rent</span></p>
             <strong class="text-uppercase">{{$houseDetails->StandardRent}}</strong>
             <hr>
             
             <p class="m-0"> <span>last billed</span></p>
             <strong class="text-uppercase">{{$houseDetails->LastBillMonth}}</strong>
             <hr>
            <div class=" text-right amount-container">
                 <p><span class="text-uppercase text-left">current balance</span> <strong> {{$houseDetails->CurrentBalanc}}</strong></p>
             </div>
             <hr>
             
             <form action="{{route('initiate-house-payment')}}" method="POST">
               @csrf
              ...
              <input type="text" class="form-control" id="mpesa-number" placeholder="eg 0712..." name="PhoneNumber" value="{{old('PhoneNumber')}}">

              <input type="hidden" class="form-control" name="HouseTypeID" value="{{$houseDetails->HouseTypeID}}">

              <input type="hidden" class="form-control" name="EstateID" value="{{$houseDetails->EstateID}}">
              <input type="hidden" class="form-control" name="UHN" value="{{$houseDetails->UHN}}">
            </div>
