      ...
      <section class="container p-0 m-0">
        ...
        <div class="row px-5 greybg">
          ...
          <div class="header-title text-uppercase p-3 pt-3 mx-5 col-8 bg-primary text-white">
                <h2 class="mt-2"><strong>Rent bill</strong></h2>
                <p>customer copy</p>
              </div>
        </div>
          ...
          <p class="  font-22">{{date('d-m-Y')}}
              ...
              <div class="col-3 text-nowrap text-right   float-right pt-4"> <small><strong>Bill Number</strong></small>
                <p class="  font-22">{{$bill->response_data->bill_number}}</p>
              </div>
          </div>
      </div>
          <!--        logo container-->
...
<!--    the main section-->
     <section class="container px-5  m-0  ">
       ...
       <h6 class="  serial-init font-22 mb-0">{{$bill->response_data->client_name}}</h6>
       <!-- <p class="m-0 p-0"><strong class="weight-600">Identifier: {{$bill->response_data->identifier}}</strong></p> -->
       </div>
          ...
          <table class="table table-borderless mt-4 billing-table px-5">
            ...
            <tbody>
              ...
              <tr class=" ">
                ...
                <td>{{$bill->response_data->brief_description}}</td>
                <td class="text-uppercase txt-primary pr-0 text-right amount">{{number_format($bill->response_data->bill_amount)}}</td>
              </tr>
            </tbody>
          </table>
       </div>
...
<!--       payment summarry-->
       <div class="row px-5 greybg payment_options">
          ...
          <table class=" ">
            <tbody>
              <tr>
                ...
                <td class="py-0"><strong>367776 <br> {{$bill->response_data->payment_code}}</strong>
            </tr>
            <!-- <tr><td class="py-0 pl-0">M-PESA account no.</td><td class="text-uppercase py-0"><strong>{{$bill->response_data->payment_code}}</strong></td></tr> -->
          ...
          <strong class="txt-primary font-21 ">KES {{number_format($bill->response_data->bill_amount)}}</strong>
...
<!--       footer-->
        ...
        <div class="col-12 ">
          <p class="">
            <span>Billed By <strong>{{Session::get('resource')['username']}}</strong>||<strong>{{$bill->response_data->payment_code}}</strong>||</span>
   ...
   <header class="mt-2">
      <section class="container p-0 m-0">
        ...
        <div class="row px-5 greybg">
             <div class="col-6 pl-5">
          <div class="header-title text-uppercase p-3 pt-3 mx-5 col-8 bg-primary text-white">
                <h2 class=""><strong>Rent bill</strong></h2>
                <p>file copy</p>
              </div>
        </div>
...
<!--    the main section-->
     <section class="container px-5  m-0  ">
       ...
       <h6 class="  serial-init font-22 mb-0">{{$bill->response_data->client_name}}</h6>
       <!-- <p class="m-0 p-0"><strong class="weight-600">Bill identifier: {{$bill->response_data->identifier}}</strong></p> -->
       </div>
       ...
       <div class="px-5 mx-5">
          <table class="table table-borderless mt-4 billing-table px-5">
            ...
            <tbody>
              <tr class=" ">
                <td scope="row" class="pl-0">1</td>
                <td scope="row" class="pl-0">{{$bill->response_data->account_number}}</td>
                <td>{{$bill->response_data->brief_description}}</td>
                <td class="text-uppercase txt-primary pr-0 text-right amount">{{number_format($bill->response_data->bill_amount)}}</td>
              </tr>
