      ...
      <div class="service-form-container d-flex flex-column-md animated col-12 p-0 ">
        <div class="col-lg-7 service-form-img-container p-0" id="parking-img">
          <div class="col-lg-8 col-md-12 position-relative p-sm-4">
            <h2 class="mb-4 text-white">Nakuru County Government PSVs seasonal parking</h2>
            <p class="font-14 mb-3 ">Select the fleet to pay for.</p>
          </div>
        </div>
        <div class="col-lg-5 p-5 position-relative transactions-form-container d-flex p-sm-4">
    	     <div class="col-12 p-0 the-transaction-form animated">
				 <div class="">
            <p class="mb-5 pb-2"><strong>Select the fleet to pay for: </strong></p>
          </div>
          <div class="transaction-form">
              ...
              <div >
                  ...
                  <form action="{{route('get-fleet-psvs')}}" method="post" id='fleet-{{$fleet->id}}'>
                    @csrf
                    <input type="hidden" name="id" value="{{$fleet->id}}">
                  </form>
                  <div class="fleet coral-light-bg" onclick="document.getElementById('fleet-{{$fleet->id}}').submit();">Fleet name: <strong>{{$fleet->name}}</strong> <br> <span><b>{{$fleet->vehicle_count}}</b> vehicles</span></div><br>
                  @endforeach
                  @else
                  <div class="alert alert-danger">Your company is not registered.</div>
                  @endif
              </div>
              

              <div id="vehicles" class="show-vehicles d-none">
        ...
        <div>
			  ...
			  <div class="col-12 p-0 cars-container mt-3">
			  	@foreach($vehicles->response_data->parking_entries as $vehicle)
			  	<span class="font-12 cars-add">
			  		<span class="number-plate" id="{{$vehicle->id}}">{{$vehicle->number_plate}}
          ...
          @if(!empty($vehicles->response_data->parking_entries))
           @foreach($vehicles->response_data->parking_entries as $vehicle)
						<div class="col-12 d-flex p-0 single-listing justify-content-between">
						 	 <div>
						 	<strong id="{{$vehicle->id}}">{{$vehicle->number_plate}}</strong>
					 	<p class="m-0"><span>{{$vehicle->vehicle_category}}</span></p>
						 </div>
						 ...
						 <div class="amount-close d-flex align-items-center text-right">
							 ...
							 <p class="m-0"><span class="font-12">{{$vehicle->duration}}</span></p>
							 </div>
             ...
             @endforeach
             @endif
						@if(!empty($vehicles->response_data))
						<div class=" text-right amount-container">                     
              <p><span class="text-uppercase text-left" >Total: </span> <strong> KES <span id="charges">{{number_format($vehicles->response_data->total_cost)}}</span></strong></p>
...
<!-- <script type="text/javascript">
  $("#car_type,#parking_duration").on("change paste keyup", function() {
      var vehicle_description = $("select[name=vehicle_description]").val();



      var duration_code = $("select[name=duration_code]").val();
      console.log(vehicle_description);
      console.log(duration_code);

      if(vehicle_description == "" || duration_code == "")
      {
        return;
      }
      ...
      $.ajax({
        ...
        data: {vehicle_description:vehicle_description, duration_code:duration_code},
...
<script type="text/javascript">
    $("#add_vehicle").click(function(e){
      ...
      var vehicle_type = $("select[name=vehicle_description] option:selected").text(); 
      ...
      $.ajax({
        ...
        type: "POST",
        headers:{'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        data: {registration_no:registration_no, duration_code:duration_code, vehicle_description:vehicle_description ,phone_number:phone_number, amount:amount, vehicle_type:vehicle_type, duration:duration},
