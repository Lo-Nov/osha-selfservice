...
<body>
  <div class="container-fluid">
    ...
    <div class="main row">
        ...
        <div class="row">
          ...
          <div class="sec1Right pr-50px col-7">
            <div class="receiptNo">
              ...
              <p class="secPContent" id="receipt">{{$receipt->receipt_number}}</p>
              <div class="barImg">
                {!! QRCode::size(150)->generate($receipt->receipt_number); !!}
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="interSecDet container">
        <div class="interSecDetContent row">
          ...
          <p class="timeDet col-3 text-center">{{$receipt->receipt_date}}</p>
          <p class="totalDet col-4 text-right pr-50px"  id="ksh">** {{$amount_in_words}} shillings **</p>
        </div>
      </div>
      ...
      <div class="slantMain container">
        <div class="row align-items-start">
          ...
          <div class="col-4 px-0">
            <p class="kshHead text-right pr-50px">KES {{number_format($receipt->total_cost)}}</p>
            <p class="kshHead text-right pr-50px">KES {{number_format($receipt->total_payments)}}</p>
            <p class="kshHead text-right pr-50px">KES {{number_format($receipt->total_cost - $receipt->total_payments)}}</p>
          </div>
        </div>
      </div>
      ...
      <footer>
        <div class="bill container">
          <div class="row">
            ...
            <div class="col-6">
              <!-- <p class="text-right">Bill No: <span class="billNo">###</span></p> -->
            </div>
          </div>
        </div>
    ...
    <!-- <a href="{{route('download-receipt', ['reference_number' => $receipt->receipt_number])}}">Download Receipt</a> -->
  </div>
